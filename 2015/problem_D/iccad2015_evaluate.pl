...
{
    print "Usage:\n";
    print "   $0 <.fp> <.aux> (Example: iccad2015_evaluate.pl adaptec1.fp adaptec1.aux)\n";
    exit(0);
}
...
writeEvalFile(\%PL,$chip_info,$nodefile,$netfile,$wtsfile,$out_plfile,$out_sclfile,$shapefile,$out_routefile);
runNTUplace3($eval_aux);
...
#---------------------------------------------------------------------------------------------------------------#
sub writeEvalFile{
    my ($plRef,$chip_info,$nodefile,$netfile,$wtsfile,$out_plfile,$out_sclfile,$shapefile,$out_routefile) = @_;
    writeAux($out_auxfile, $nodefile,$netfile,$wtsfile,$out_plfile,$out_sclfile,$shapefile,$out_routefile);
    writePl(\%plRef, $out_plfile);
    writeScl(\%chip_info, $out_sclfile);
    writeRoute(\%plRef, \%chip_info, $out_routefile);
}
#---------------------------------------------------------------------------------------------------------------#
sub runNTUplace3{
    my ($eval_aux) = @_;
    print "Executing NTUplace3...\n";
    system("./ntuplace3 -aux $eval_aux");
}
#---------------------------------------------------------------------------------------------------------------#
sub runNCTUgr{
    my ($eval_aux, $ntu_out_file, $nctu_setting_file, $nctu_out_file) = @_;
    print "Running NCTUgr...\n";
    system("./NCTUgr ICCAD $eval_aux $ntu_out_file $nctu_setting_file $nctu_out_file");
}
#---------------------------------------------------------------------------------------------------------------#
sub evalRoutability{
    my ($eval_aux, $ntu_out_file, $nctu_out_file) = @_;
    print "Running Routability Evaluation...\n";
    system("perl evaluate_routability.pl -p $eval_aux $ntu_out_file $nctu_out_file");
}
#---------------------------------------------------------------------------------------------------------------#
sub checkDesign{
    ...
    $design_site = $chip_info->{TRX} * $chip_info->{TRY};
   ...
   #}
    ...
    #foreach my $keyA (keys %plRef){
    #    if($plRef->{$keyA}->{fixed} eq 'true') {
    #        foreach my $keyB (keys %plRef){
    #            if($plRef->{$keyB}->{fixed} eq 'true') {
    #                my $ret = rectOverlap(\%ptA, \%ptB);
    #                $invalid = $invalid + $ret;
    #            }
    #        }
    #    }
    #}
    #printf("Invalid Macro Placement : %d\n",$invalid);
}
#---------------------------------------------------------------------------------------------------------------#
...
sub rectOverlap {
    ...
    my $aBLX = $rectA->{x};
    my $aBLY = $rectA->{y};
    my $aTRX = $rectA->{x} + $rectA->{w};
    my $aTRY = $rectA->{y} + $rectA->{h};
    my $bBLX = $rectB->{x};
    my $bBLY = $rectB->{y};
    my $bTRX = $rectB->{x} + $rectB->{w};
    my $bTRY = $rectB->{y} + $rectB->{h};
    my $ret = 1;
    if ( ($aBLX >= $bTRX) || ($aTRX <= $aBLX) ) {
        $ret = 0;
        return $ret;
    }
    if ( ($aBLY >= $bTRY) || ($aTRY <= $aBLY) ) {
        $ret = 0;
        return $ret;
    }
    return $ret;
...
sub calcChipInfo{
    ...
    $design_sites = $chip_info->{TRX} * $chip_info->{TRY};
...
#---------------------------------------------------------------------------------------------------------------#
sub writeScl{
    my ( $chip_info, $out_file) = @_;
    
    $margin = 0.01;
    $row_height = 12;
    $num_row = ceil( (($chip_info->{TRY} - $chip_info->{BLY})/12 ) * (1.00 + $margin) );
    ...
    for(my $iRow = 0; $iRow < $num_row; $iRow++) {
        $coordinate = $chip_info->{BLX} + $iRow * 12;
    ...
    close $OUTFILE;
    printf("Generating .scl file   : %s\n",$out_file);
} 
#---------------------------------------------------------------------------------------------------------------#
sub writeRoute{
    my ($pl, $chip_info, $out_file) = @_;
    ...
    $datestring = localtime();
    open(my $OUTFILE, '>', $out_file) or die "Could not open file '$out_file' $!";
    #$grid_x = ceil( (($chip_info->{TRX} - $chip_info->{BLX})/$tile_w ) * (1.00 + $margin) );
    #$grid_y = ceil( (($chip_info->{TRY} - $chip_info->{BLY})/$tile_h ) * (1.00 + $margin) );
    ...
    printf $OUTFILE "\n";
    close $OUTFILE;
    printf("Generating .route file : %s\n",$out_file);
}
#---------------------------------------------------------------------------------------------------------------#
sub calcScale{
    my ($PlaneSize) = @_;
