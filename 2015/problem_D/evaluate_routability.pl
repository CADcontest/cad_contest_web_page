...
# --------------------------------------------------------------------------------------
# ICCAD 2012 Routability-driven Placement Contest Evaluation Script
# --------------------------------------------------------------------------------------
# Author:
#   Natarajan Viswanathan, IBM Corp., Austin, TX (nviswan@us.ibm.com)
#   Yaoguang Wei, IBM Corp., Austin, TX (weiyg@us.ibm.com)
# Usage:
#   iccad2012_evaluate_solution.pl <circuit.aux> <placement solution> <routing solution>
# --------------------------------------------------------------------------------------
# NOTES:
...
#       which is N. For the ICCAD 2012 contest, no flipping / mirroring / rotation 
...
if(($#ARGV < 2) || ($opt_h))
{
  ...
  print "               (ICCAD 2012 contest evaluation metric excluding runtime)\n";
  exit(0);
}
...
############## Main ##############
{
  ...
  my %PL;
  my %chip_info;
  readPl(\%PL, $place_file);
  readNode(\%PL, $DesignDB{nodes});
  calcChipInfo(\%PL, \%chip_info);
  $MetricsDB{AREA} = $chip_info{area};
...
sub read_route_file {
  ...
  my ($temp, @temp, $num_ni_terminals, $num_route_blockages, $num_edge_capacity_adjustments);
  ...
  while(<ROUTEFILE>)
  {
    ...
    elsif(/^(NumEdgeCapacityAdjustments)/)
    {
      ($temp, $num_edge_capacity_adjustments) = split ":";
      $num_edge_capacity_adjustments = process_line($num_edge_capacity_adjustments);
      
      next if($num_edge_capacity_adjustments == 0);
      
      # get the capacity adjustments for the tile edges
      # NOTE: This section gives the actual capacity for a given global routing tile edge
      #       The global routing tile edges defined in this section will always be mutually 
      #       exclusive of the tile edges being blocked/overlapped by the blockage nodes
      my $num_adjust_records = 0;
      while(<ROUTEFILE>)
      {
        $_ = process_line($_);
        if(/^\s*$/ or /^#.*/)
        ...
        else
        {
          $num_adjust_records++;
          my (@tmpRecord) = split;
          $ref_RouteDB->{edge_cap_adjustment}{$num_adjust_records} = \@tmpRecord;
          last if($num_adjust_records == $num_edge_capacity_adjustments);
        }
      }
    }
    else
  ...
  if($VERBOSE)
  {
    ...
    print "         Num Edge Capacity Adjustments: $num_edge_capacity_adjustments\n";
  }
}
...
sub get_tile_capacity {
  ...
  # process the tile edge capacity adjustment section
  my $num_edge_cap_adjustment = scalar(keys(%{$ref_RouteDB->{edge_cap_adjustment}}));
  
  my $num_adjust_record = 0;
  if(scalar(keys(%{$ref_RouteDB->{edge_cap_adjustment}})) > 0)
  {
    # get the edge capacity adjustment for the current tile edge
    foreach my $name (keys(%{$ref_RouteDB->{edge_cap_adjustment}}))
    {
      $num_adjust_record++;
      my @tmpRecord = @{$ref_RouteDB->{edge_cap_adjustment}{$name}};
      # each record has the data as follows:
      # col row layer col row layer reduced_capacity
      # col=[0..$grid_x-1], row=[0..$grid_y-1], layer=[1..$grid_z]
      my $col_1 = $tmpRecord[0];
      my $row_1 = $tmpRecord[1];
      my $lay_1 = $tmpRecord[2];
      my $col_2 = $tmpRecord[3];
      my $row_2 = $tmpRecord[4];
      my $lay_2 = $tmpRecord[5];
      my $adjusted_capacity = $tmpRecord[6];
      
      die "Incorrect layer information ($lay_1, $lay_2).\n"
      if($lay_1 != $lay_2);
      
      if($row_1 == $row_2)
      # reduce capacity for horizontal routes
      {
        die "Max H_Capacity for layer($lay_1) is zero.\n"
        if($h_capacity[$lay_1-1] <= 0);
        die "Incorrect column information ($col_1, $col_2) for H adjust.\n"
        if($col_2 != $col_1+1);
        
        my $max_capacity = $h_capacity[$lay_1-1];
        my $wire_width   = $wire_width[$lay_1-1];
        my $wire_spacing = $wire_spacing[$lay_1-1];
        my $num_tracks = int($adjusted_capacity / ($wire_width + $wire_spacing));
        $adjusted_capacity = $num_tracks * ($wire_width + $wire_spacing);
        $adjusted_capacity = min($max_capacity, max(0, $adjusted_capacity));
        
        $capr[$col_1][$row_1][$lay_1-1] = $adjusted_capacity;
        $capl[$col_2][$row_2][$lay_2-1] = $adjusted_capacity;
      }
      elsif($col_1 == $col_2)
      # reduce capacity for vertical routes 
      {
        die "Max V_Capacity for layer($lay_1) is zero.\n"
        if($v_capacity[$lay_1-1] <= 0);
        die "Incorrect row information ($row_1, $row_2) for V adjust.\n"
        if($row_2 != $row_1+1);
        
        my $max_capacity = $v_capacity[$lay_1-1];
        my $wire_width   = $wire_width[$lay_1-1];
        my $wire_spacing = $wire_spacing[$lay_1-1];
        my $num_tracks = int($adjusted_capacity / ($wire_width + $wire_spacing));
        $adjusted_capacity = $num_tracks * ($wire_width + $wire_spacing);
        ...
        $capt[$col_1][$row_1][$lay_1-1] = $adjusted_capacity;
        $capb[$col_2][$row_2][$lay_2-1] = $adjusted_capacity;
      }
      else
      {
        die "Incorrect capacity adjustment record ($name).\n";
      }
    }
  }
  
  die "Num_Edge_Cap_Adjustment($num_edge_cap_adjustment) != Num_Adjust_Records($num_adjust_record)\n"
  if($num_edge_cap_adjustment != $num_adjust_record);
  ...
  if($VERBOSE)
  {
    ...
    print "         Num Capacity Adjustments due to blockages  : $num_cap_adjustment\n";
    print "         Num Edge Capacity Adjustments : $num_edge_cap_adjustment\n";
  }
...
sub get_congestion_stats {
  ...
  if($VERBOSE)
  {
    ...
    printf("         Ignore edges with blockages above  : %.1lf%%\n", 100*$IGNORE_EDGE_RATIO);
    printf("         Pin blockage ratio                 : %.2lf\n", $PIN_BLOCKAGE_FACTOR);
    printf("         ACE percentage array               : @ACE_PERCENT_ARRAY\n");
    printf("         Congestion weight                  : @CONGESTION_WEIGHT\n");
    printf("         Routing congestion penalty factor  : %.2lf\n", $PENALTY_FACTOR);
  }
...
sub print_metrics {
  ...
  # Scaled Wire Length
  my $Scaled_WL   = $ref_MetricsDB->{HPWL}*(1.0 + $PENALTY_FACTOR*($rc - 100.0));
  my $Scaled_AREA = $ref_MetricsDB->{AREA}*(1.0 + $PENALTY_FACTOR*($rc - 100.0));
  
  print "Total Half Perimeter Wire Length : $ref_MetricsDB->{HPWL}\n";
  print "Macro Placement Area             : $ref_MetricsDB->{AREA}\n";
  ...
  printf("Scaled Area : %.0lf\n", $Scaled_AREA);
  
  printf("RESULTS:  ");
  printf("%.2lf   ", 100*$_) foreach(@maxAceArr);
  printf("%.2lf   ", $rc);
  printf("%.0lf   ", $ref_MetricsDB->{HPWL});
  printf("%.0lf   ", $Scaled_WL);
  print "\n";
}
...
sub plot_congestion_maps {
  ...
  if($core_width > $core_height) {
    $x_size = 1000;
    $y_size = int(($x_size*$core_height)/$core_width);
  } else {
    $y_size = 1000;
    $x_size = int(($y_size*$core_width)/$core_height);
  }
  $x_size += 50;
  ...
  for $z (0 .. ($grid_z-1))
  {
    ...
    print OUTFILE "set terminal png crop truecolor size $x_size, $y_size\n";
    print OUTFILE "set output \"$design.M$layer_id.congestion.png\"\n";
    print OUTFILE "splot [0:$grid_x][0:$grid_y] \'-\' matrix\n";
